...
void scheduler(void)
{
  ...
  for (;;)
  {
    ...
    printf("INSIDE FCFS\n");
    int time_min = 100000000;

    for (cand_p = proc; cand_p < &proc[NPROC]; cand_p++)
    {
      acquire(&cand_p->lock);
      if (cand_p->state == RUNNABLE && (cand_p->ctime) < time_min)
      {
        // Switch to chosen process.  It is the process's job
        // to release its lock and then reacquire it
        // before jumping back to us.
        done = 1;
        time_min = cand_p->ctime;
        p = cand_p;
  ...
  for (;;)
  {
    ...
    for (cand_p = proc; cand_p < &proc[NPROC]; cand_p++)
    {
      ...
      if (cand_p->state == RUNNABLE && (cand_p->ctime) < time_min)
      {
        ...
        // to release its lock and then reacquire it
        // before jumping back to us.
        printf("C_TIME IS %d", cand_p->ctime);
